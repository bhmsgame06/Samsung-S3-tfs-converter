...
public class Converter {
	...
	public static boolean initFolders() {
		boolean r = false;
		File src;
		if(!(src = new File(FOLDER_DEFAULT + "/S3-tfs")).exists()) {
			src.mkdir();
			r = true;
		}
		if(!(src = new File(FOLDER_DEFAULT + "/S3-tfs/tfs")).exists()) src.mkdir();
		if(!(src = new File(FOLDER_DEFAULT + "/S3-tfs/ext")).exists()) src.mkdir();
		return r;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		if(MADE) out.println("No program files were found on your disk, new files were created. Path: " + FOLDER_DEFAULT + "/S3-tfs");
		if(args.length < 1) {
		...
		try {
			tfs = new DataInputStream(new FileInputStream(FOLDER_DEFAULT + "/S3-tfs/tfs/" + args[0]));
			ext = new ZipOutputStream(new FileOutputStream(FOLDER_DEFAULT + "/S3-tfs/ext/" + args[0] + ".zip"));
		} catch(Exception e) {
	...
	private static void extract(int size) throws IOException {
